...
rule gtdb_prep:
    ...
    retries: 3
    log:
        "logs/gtdb/gtdb_prep/gtdb_prep-{accession}.log",
    conda:
    ...
    shell:
        """
        if python workflow/bgcflow/bgcflow/data/gtdb_prep.py {wildcards.accession} {output.gtdb_json} {input.samples_path} '{params.gtdb_paths}' {params.version} {params.offline} {params.api_base} 2> {log}; then
            ...
            python workflow/bgcflow/bgcflow/data/gtdb_prep_from_table.py {wildcards.accession} resources/gtdb_download/{params.gtdb_table} {params.version} {output.gtdb_json} 2>> {log}
        fi
        """
...
checkpoint fix_gtdb_taxonomy:
    input:
        json_list=lambda wildcards: [f"data/interim/gtdb/{accession}.json" for accession in get_accessions_for_taxon(wildcards.taxon)],
    output:
    ...
    shell:
        """
        TMPDIR="data/interim/tmp/{wildcards.taxon}"
        mkdir -p $TMPDIR
